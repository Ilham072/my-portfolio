...
export default function AdminPortfolioIndex({ items }: { items: Paginated<Article> }) {
  const confirm = useConfirm();
  const [target, setTarget] = React.useState<Article | null>(null);
  const [loading, setLoading] = React.useState(false);

  const askDelete = (p: Article) => {
    setTarget(p);
    confirm.confirm();
  };
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href={`${routes.admin.articles}/create`} className="rounded-xl bg-zinc-900 px-4 py-2 text-sm font-medium text-white">
          New
        </Link>
      </div>
            ...
            {items.data.map((p) => (
              <tr key={p.id} className="border-b">
                ...
                <td className="p-3 flex gap-2">
                  <Link className="text-zinc-900 underline" href={`${routes.admin.articles}/${p.id}/edit`}>
                    Edit
                  </Link>
